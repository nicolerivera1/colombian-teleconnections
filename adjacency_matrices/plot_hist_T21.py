...
def get_data(varname, index):
    ...
    # import nc file
    DS = xr.open_dataset(
        "col-inf-transf-data-v1/T-col-milinats/T_" + index + "_" + varname + ".nc"
    )
    ds = DS.to_dataframe()
    arr = ds[str("T_" + index + "_" + varname)].to_numpy()
...
def plot_hist_from_data(index_name, var_name, save=False):
    ...
    if index_name == "nino3":
        b.set_xlabel(
            '\n'+r"$T_{Y \rightarrow %s } \ (\frac{milinats}{month})$" % var_name.upper(), fontsize=26, weight="bold"
    ...
    if save:
        plt.savefig("hist-inf-transf/" + index_name + "_" + var_name + "_hist.png")
