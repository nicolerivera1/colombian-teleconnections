...
def get_variables_all_indexes(
    ...
    for index in indexlist:
        # import nc file
        DS = xr.open_dataset(
            "col-inf-transf-data-v1/T_" + index + "_" + varname + ".nc"
        )
        ds = DS.to_dataframe()
        # ds.rename(columns={"tau21": index.upper()}, inplace=True)
        ds[str("T_" + index + "_" + varname)] *= 1e3  # from nats to milinats
...
def all_variables_boxplot(df, title, save_fig=False):
    ...
    b.set_ylabel(
        r'Information Flow ($\frac{milinats}{month}$)', fontsize=14, weight="bold"
    ...
    if save_fig:
        fig = b.get_figure()
        fig.savefig("imgs/inf_transf_" + title.replace(" ", "_").lower() + ".png")
